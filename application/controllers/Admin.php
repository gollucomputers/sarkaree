...
class Admin extends CI_Controller {
    ...
    public function managecats()
   	...
   	{
   		$this->form_validation->set_rules('title', 'Tags Name', 'required|min_length[3]');
   		$data = array(
   				'title' => $this->input->post('title'),
   				'description' => $this->input->post('description')
   			);
   		if($this->form_validation->run() == false){
   			$this->load_view('admin/addtags');
   		}else{
   			if($this->adminModel->addtags($data)){
   				$this->session->set_flashdata('added','Tags added Successfully');
   				$this->load_view('admin/addtags');
   			}
   		}
    ...
    public function managetags()
   	{
   		# here you can manage news tags
   		$data['tags'] = $this->adminModel->gettags();
   		$this->load_view('admin/managetags',$data);
   	...
   	{
   		# code...
   		$tags_id = $this->uri->segment(3);
   		$this->form_validation->set_rules('title', 'Tags Name', 'required|min_length[3]');
   		if($this->form_validation->run() == false){
   			$data['tagsdetails'] = $this->adminModel->gettagswhere($tags_id);
   			$this->load_view('admin/edittags',$data);
   		}else{
   			$data = array(
   				'title' => $this->input->post('title'),
   				'description' => $this->input->post('description')
   			);
   			if($this->adminModel->edittags($tags_id,$data)){
   				$this->session->set_flashdata('edited','Tags edited Successfully');
   				$data['tagsdetails'] = $this->adminModel->gettagswhere($tags_id);
   				$this->load_view('admin/edittags',$data);
   			}
   		}
   	...
   	{
   		# code...
   		if ($this->adminModel->deletetags($tags_id)) {
   			$this->session->set_flashdata('deleted','Tags deleted Successfully');
   			redirect('admin/managetags');
   		}
   	}
            ...
            if (!$this->upload->do_upload('image'))
                ...
                else
                {
                  ...
                  $tags = array(
                  	...
                  	if($this->adminModel->addnews($insert, $tags)){ //if inserting is done
          ...
          $this->load_view('admin/editnews',$data);
              ...
              $tags = array(
         		'tags' => $this->input->post('tags'),
                    ...
                    if($this->adminModel->editnews($newsid,$insert, $tags)){ //if updating is done
